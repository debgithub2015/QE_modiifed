...
!
  ...
  !
  ! DC 02/15/2018
  !              "b86n" revised Becke (b86b)          igcx =41
  !                        kappa=0.81664352
  ! DC 02/25/2019
  !             "OB8N" revised optb88 exchange        igcx =42
  !                        beta=mu_PBEsol/1.70
...
! DC added
  data gradx / 'NOGX', 'B88', 'GGX', 'PBX',  'RPB', 'HCTH', 'OPTX',&
               ...
               'xxxx', 'xxxx', 'xxxx', 'xxxx', 'BR0', 'xxxx', 'C090','B86N',&
               'OB8N' /
...
! DC 02/16/2018
  elseif (igcx == 41) then ! 'b86n'
     call b86b (rho, grho, 4, sx, v1x, v2x)
! DC 02/25/2019
  elseif (igcx == 42) then ! 'OB8N'
...
! DC 02/16/2018
   elseif (igcx == 41) then ! 'b86n'
     if (rhoup > small .and. sqrt (abs (grhoup2) ) > small) then
        call b86b (2.0_DP * rhoup, 4.0_DP * grhoup2, 4, sxup, v1xup, v2xup)
     else
        sxup = 0.0_DP
        v1xup = 0.0_DP
        v2xup = 0.0_DP
     ...
     if (rhodw > small .and. sqrt (abs (grhodw2) ) > small) then
        call b86b (2.0_DP * rhodw, 4.0_DP * grhodw2, 4, sxdw, v1xdw, v2xdw)
     else
        sxdw = 0.0_DP
        v1xdw = 0.0_DP
        v2xdw = 0.0_DP
...
! DC 02/16/2018
  case(41) ! 'b86n'
     do i=1,length
        if (rhoup(i) > small .and. sqrt(abs(grhoup2(i))) > small) then
           call b86b (2.0_DP * rhoup(i), 4.0_DP * grhoup2(i), 4, sxup(i), v1xup(i), v2xup(i))
        else
           sxup(i) = 0.0_DP
           v1xup(i) = 0.0_DP
           v2xup(i) = 0.0_DP
        ...
        if (rhodw(i) > small .and. sqrt(abs(grhodw2(i))) > small) then
           call b86b (2.0_DP * rhodw(i), 4.0_DP * grhodw2(i), 4, sxdw(i), v1xdw(i), v2xdw(i))
        else
           sxdw(i) = 0.0_DP
           v1xdw(i) = 0.0_DP
           v2xdw(i) = 0.0_DP
