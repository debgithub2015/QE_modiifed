...
! DC adds here 12/20/2019
  !! iflag=8  Klimes, Bowler, Michaelides, PRB 83, 195131 (2011)
  !                       beta=mu_PBEsol/1.10 
  !
  USE kinds,      ONLY : DP
!  USE input_parameters, ONLY:mu_b88,kappa_b88,c_b88
  !
  IMPLICIT NONE
  !
  ...
  !DC added 03/04/2020
  REAL(DP) :: mu_b88,kappa_b88,c_b88
...
SUBROUTINE b86b( rho, grho, iflag, sx, v1x, v2x )                    !<GPU:DEVICE>
  ...
  !
  USE kinds,     ONLY : DP
!  USE input_parameters, ONLY: mu_b86r,beta_b86r,power_b86r 
  IMPLICIT NONE
  !
  INTEGER, INTENT(IN) :: iflag                  !<GPU:VALUE>
  REAL(DP), INTENT(IN) :: rho, grho
  ...
  !DC added 03/04/2020
  REAL(DP) :: mu_b86r,beta_b86r,power_b86r 
  !
  agrho = SQRT(grho)
  kf = c2 * rho**third
  dsg = 0.5_DP / kf
  ...
  ! ... Energy
  !
!DC added on 03/04/2020
  IF ( iflag == 4 ) THEN
         f1 = mu_b86r*s2
         f2 = 1._DP + mu_b86r*s2/beta_b86r
         f3 = f2**power_b86r
  ELSE              
         f1 = mu(iflag)*s2
  ...
  !
  dxunif = exunif * third
! DC added on 03/03/2020
  IF ( iflag == 4 ) THEN
        dfx1 = 1.d0 + (1.d0 - power_b86r)*mu_b86r*s2/beta_b86r
        dfx  = 2._DP * mu_b86r * s1 * dfx1 / (f2 * f3)
  ELSE
