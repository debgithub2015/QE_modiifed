...
PUBLIC  :: xc_vdW_DF, xc_vdW_DF_spin, vdW_DF_stress,                   &
           vdW_DF_energy, vdW_DF_energy_3ord,                     &
           vdW_DF_potential, vdW_DF_potential_3ord,                &
           generate_kernel, generate_kernel_3rd_order,                 &
           interpolate_kernel,                                         &
...
!DC added
INTEGER,  PARAMETER                 :: Nqs_3ord    = 6
REAL(DP), PARAMETER, DIMENSION(Nqs_3ord) :: q_mesh_3ord = (/  &
   q_min              , 0.05D0, 0.5D0, 1.50D0, 3.0D0, q_cut /)

! The above two parameters define the q mesh to be used in the vdW_DF
! code. These are perhaps the most important to have set correctly.
! Increasing the number of q points will DRAMATICALLY increase the
! memory usage of the vdW_DF code because the memory consumption depends
! cubically on the number of q points in the mesh. Increasing the
! number of q points might increase accuracy of the vdW_DF code. The largest value of
! the q mesh is q_cut. All values of q0 (DION equation 11) larger than
! this value during a run will be saturated to this value using equation
! 5 of SOLER. In testing, increasing the value of q_cut was found to
! have little impact on the results, although it is possible that in
...
!DC: 01/14/2020: Defined for 3rd order kernel
REAL(DP) :: kernel3ord( 0:Nr_points, Nqs_3ord, Nqs_3ord, Nqs_3ord ), d2phi3ord_dk2( 0:Nr_points, Nqs_3ord, Nqs_3ord, Nqs_3ord )


REAL(DP) :: W_ab( Nintegration_points, Nintegration_points )
! Defined in DION equation 16.

!DC : 01/14/2020 : Defined for 3rd order expansion
REAL(DP) :: G1_abc(Nintegration_points, Nintegration_points, Nintegration_points) 
REAL(DP) :: G2_abc(Nintegration_points, Nintegration_points, Nintegration_points)
REAL(DP) :: G3_abc(Nintegration_points, Nintegration_points, Nintegration_points)
...
!DC added
  REAL(DP), ALLOCATABLE :: potential(:),potential_3ord(:)   ! The vdW contribution to the potential.
  ...
  COMPLEX(DP), ALLOCATABLE :: thetas_3ord(:,:)   ! These are the functions of equation 8 of
                                                 ! SOLER. They will be forward Fourier transformed
                                                 ! in place to get theta(k) and worked on in
                                                 ! place to get the u_alpha(r) of equation 11
                                                 ! in SOLER. They are formatted as follows:
  ...
  REAL(DP) :: Ec_nl, Ec_nl_3ord                  ! The non-local vdW contribution to the energy.
...
!DC added
     CALL generate_kernel_3rd_order
...
!DC added
  allocate( total_rho(dfftp%nnr), grad_rho(3,dfftp%nnr),                &
            potential(dfftp%nnr), potential_3ord(dfftp%nnr),            &
            thetas(dfftp%nnr, Nqs), thetas_3ord(dfftp%nnr, Nqs_3ord),    &
...
!DC added
  CALL get_q0_on_grid (total_rho, grad_rho, q0, dq0_drho, dq0_dgradrho, thetas_3ord)
  CALL vdW_DF_energy_3ord (thetas_3ord, Ec_nl_3ord )
  etxc= etxc + Ec_nl_3ord

  IF ( iverbosity > 0 ) THEN
     CALL mp_sum(Ec_nl_3ord, intra_bgrp_comm)
     IF ( ionode ) THEN
        WRITE(stdout,'(/ / A)')       "     -----------------------------------------------"
        WRITE(stdout,'(A, F15.8, A)') "     Non-local corr. energy 3rd order =  ", Ec_nl_3ord, " Ry"
        WRITE(stdout,'(A /)')         "     -----------------------------------------------"
     END IF
  END IF

  DO theta_i = 1, Nqs_3ord
     CALL invfft('Rho', thetas_3ord(:,theta_i), dfftp)
  END DO
  
  CALL vdW_DF_potential_3ord (q0, dq0_drho, dq0_dgradrho, grad_rho, thetas_3ord, potential_3ord)
  v(:,1) = v(:,1) + e2 * potential_3ord(:)

  DO i_grid = 1, dfftp%nnr
     vtxc = vtxc + e2 * grid_cell_volume * rho_valence(i_grid,1) * e2 * potential_3ord(i_grid)
  END DO

!DC changed
  DEALLOCATE ( total_rho, grad_rho, potential, thetas, thetas_3ord, potential_3ord, q0, dq0_drho, dq0_dgradrho )
...
!DC added
  ...
  REAL(DP), ALLOCATABLE :: potential3ord_up(:)     
  REAL(DP), ALLOCATABLE :: potential3ord_down(:)   
...
!DC added
  ...
  ALLOCATE( total_rho(dfftp%nnr), rho_up(dfftp%nnr), rho_down(dfftp%nnr),         &
     ...
     potential3ord_up(dfftp%nnr), potential3ord_down(dfftp%nnr),                  &
     q0(dfftp%nnr), dq0_drho_up(dfftp%nnr), thetas_3ord(dfftp%nnr, Nqs_3ord),     &
     dq0_dgradrho_up(dfftp%nnr), dq0_drho_down(dfftp%nnr),                        &
     dq0_dgradrho_down(dfftp%nnr) )
  ...
  !write(stdout,*) 'rho_up', rho_up, 'rho_down', rho_down, 'total_rho', total_rho
...
!DC added
  CALL get_q0_on_grid_spin (total_rho, rho_up, rho_down, grad_rho, &
       grad_rho_up, grad_rho_down, q0, dq0_drho_up, dq0_drho_down, &
       dq0_dgradrho_up, dq0_dgradrho_down, thetas_3ord)
  CALL vdW_DF_energy_3ord (thetas_3ord, Ec_nl_3ord )
  etxc= etxc + Ec_nl_3ord
  ...
  CALL vdW_DF_potential_3ord (q0, dq0_drho_up  , dq0_dgradrho_up  , grad_rho_up  , thetas_3ord, potential3ord_up  )
  CALL vdW_DF_potential_3ord (q0, dq0_drho_down, dq0_dgradrho_down, grad_rho_down, thetas_3ord, potential3ord_down)

  v(:,1) = v(:,1) + e2 * potential3ord_up  (:)
  v(:,2) = v(:,2) + e2 * potential3ord_down(:)

  DO i_grid = 1, dfftp%nnr
     vtxc = vtxc + e2 * grid_cell_volume * (rho_valence(i_grid,1) +     &
            rho_valence(i_grid,2)) * 0.5_dp * potential3ord_up(i_grid)  &
                 + e2 * grid_cell_volume * (rho_valence(i_grid,1) -     &
            rho_valence(i_grid,2)) * 0.5_dp * potential3ord_down(i_grid)
  END DO

!DC changed
  DEALLOCATE( total_rho, rho_up, rho_down, grad_rho, grad_rho_up, grad_rho_down,   &
              potential_up, potential_down, thetas, potential3ord_up,              &
              potential3ord_down, thetas_3ord,                                     &
...
!DC added
  INTEGER,PARAMETER         :: N_int = 256
  REAL(dp)                  :: abscissas(N_int), weights(N_int)
  REAL(dp)                  :: C6,C8,C9,C10,prefac,alpha1,alpha2,alpha3
  REAL(dp)                  :: ab2,beta,sigma,sigma_3ord,delta,a,b
  REAL(dp), ALLOCATABLE     :: q_raw(:)

  ALLOCATE (q_raw(dfftp%nnr))
...
!DC adden
  ...
  !DION's prefactor
   prefac = (9.0D0/(8.0D0*pi))**2


! From 2nd and 3rd order vdW-DF
  do idx = 1, N_int
     alpha1 = 0.0D0
     ab2 = tan(abscissas(idx))**2
     do i_grid = 1, dfftp%nnr
        alpha1 = alpha1 + total_rho(i_grid)/(prefac*q_raw(i_grid)**4 + ab2)
     end do
     alpha1 = alpha1 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
     call mp_sum( alpha1, intra_bgrp_comm )
     C6 = C6 + (3.0D0/pi) * weights(idx)*(1.0D0+ab2)*alpha1*alpha1
  end do

  write(stdout,'(/ / A)')       "-----------------------------------------------"
  write(stdout,'(A, F15.8)')    "     C6 coefficient = ", C6
  write(stdout,'(A /)')         "-----------------------------------------------"
  ...
  do idx = 1, N_int
     ...
     do i_grid = 1, dfftp%nnr
        alpha1 = alpha1 + total_rho(i_grid)/(prefac*q_raw(i_grid)**4 + ab2)
        alpha2 = alpha2 + &
                 2.0D0*total_rho(i_grid)/(2.0D0*prefac*q_raw(i_grid)**4 + ab2)
     end do
     alpha1 = alpha1 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
     call mp_sum( alpha1, intra_bgrp_comm )
     alpha2 = alpha2 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
     call mp_sum( alpha2, intra_bgrp_comm )
     C8 = C8 + (15.0D0/(2.0D0*pi))*(weights(idx)*(1.0D0+ab2)*alpha1*alpha2 + &
                 weights(idx)*(1.0D0+ab2)*alpha2*alpha1)
  end do

  write(stdout,'(/ / A)')       "-----------------------------------------------"
  write(stdout,'(A, F15.8)')    "     C8 coefficient = ", C8
  write(stdout,'(A /)')         "-----------------------------------------------"
  ...
  do idx = 1, N_int
     ...
     do i_grid = 1, dfftp%nnr
        ...
        alpha2 = alpha2 + &
                 2.0D0*total_rho(i_grid)/(2.0D0*prefac*q_raw(i_grid)**4 + ab2)
        alpha3 = alpha3 + &
                 3.0D0*total_rho(i_grid)/(3.0D0*prefac*q_raw(i_grid)**4 + ab2)
     end do
     alpha1 = alpha1 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
     call mp_sum( alpha1, intra_bgrp_comm )
     alpha2 = alpha2 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
     call mp_sum( alpha2, intra_bgrp_comm )
     alpha3 = alpha3 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
     call mp_sum( alpha3, intra_bgrp_comm )
     C10 = C10 + (14.0D0/pi)*weights(idx)*(1.0D0+ab2)*alpha1*alpha3 + &
                 (35.0D0/pi)*weights(idx)*(1.0D0+ab2)*alpha2*alpha2 + &
                 (14.0D0/pi)*weights(idx)*(1.0D0+ab2)*alpha3*alpha1
  end do

  write(stdout,'(/ / A)')       "-----------------------------------------------"
  write(stdout,'(A, F15.8)')    "     C10 coefficient = ", C10
  write(stdout,'(A /)')         "-----------------------------------------------"

!The triple diplole moment in vdW-DF C9
  ...
  do idx = 1, N_int
     ...
     alpha1 = alpha1 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
     call mp_sum( alpha1, intra_bgrp_comm )
     C9 = C9 + (3.0D0/pi) * weights(idx)*(1.0D0+ab2)*alpha1*alpha1*alpha1
  end do

  write(stdout,'(/ / A)')       "-----------------------------------------------"
  write(stdout,'(A, F15.8)')    "     C9 coefficient = ", C9
  write(stdout,'(A /)')         "-----------------------------------------------"
...
!DC added
  ...
  INTEGER                    :: i_grid, j                                   ! Indexing variable.
  LOGICAL                    :: calc_qx_up, calc_qx_down

!DC added
  INTEGER, PARAMETER        :: N_int = 256
  REAL(dp)                  :: abscissas(N_int), weights(N_int)
  REAL(dp)                  :: alpha_up,alpha_down,alpha1,alpha2,alpha3,ab2,prefac
  REAL(dp)                  :: C6, C8, C9, C10, prefac_up, prefac_down
  REAL(dp)                  :: n_elec_up,n_elec_dwn
  REAL(dp), ALLOCATABLE     :: q_raw(:),q_raw_up(:),q_raw_down(:)


  allocate ( q_raw_up(dfftp%nnr) )
  allocate ( q_raw_down(dfftp%nnr))
  allocate ( q_raw(dfftp%nnr))
  ...
  DO i_grid = 1, dfftp%nnr
     ...
     q_raw_up(i_grid)   = (q_raw_up(i_grid)*up)/rho
     q_raw_down(i_grid) = (q_raw_down(i_grid)*down)/rho
     q_raw(i_grid) = q_raw_up(i_grid) + q_raw_down(i_grid) + qc
  ...
  !DION's prefactor
   prefac = (9.0D0/(8.0D0*pi))**2
    ...
    do idx = 1, N_int
       ...
       !write(stdout,*)'idx',idx, 'i_grid', i_grid, 'total_rho', total_rho(i_grid), 'q_raw',q_raw(i_grid),'alpha1',alpha1
       alpha1 = alpha1 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
       call mp_sum( alpha1, intra_bgrp_comm )
       C6 = C6 + (3.0D0/pi) * weights(idx)*(1.0D0+ab2)*alpha1*alpha1
    end do
    ...
    do idx = 1, N_int
       ...
       alpha1 = alpha1 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
       call mp_sum( alpha1, intra_bgrp_comm )
!Needs an approximation for qudrupole 
       alpha2 = alpha2 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
       call mp_sum( alpha2, intra_bgrp_comm )
       C8 = C8 + (15.0D0/(2.0D0*pi))*(weights(idx)*(1.0D0+ab2)*alpha1*alpha2 + &
                   weights(idx)*(1.0D0+ab2)*alpha2*alpha1)
...
!Needs an approximation for qudrupole 
        alpha2 = alpha2 + &
                 2.0D0*total_rho(i_grid)/(2.0D0*prefac*q_raw(i_grid)**4 + ab2)
!Needs an approximation for hexapole
        alpha3 = alpha3 + &
                 3.0D0*total_rho(i_grid)/(3.0D0*prefac*q_raw(i_grid)**4 + ab2)
     end do
     alpha1 = alpha1 * omega/( dfftp%nr1*dfftp%nr2*dfftp%nr3 )
...
! Triple dipole moment vdW-DF C9
  ...
  deallocate ( q_raw )
  deallocate ( q_raw_up )
  deallocate ( q_raw_down )

  n_elec_up=0.d0
  n_elec_dwn=0.d0
  do idx = 1, dfftp%nnr
     n_elec_up=n_elec_up+rho_up(idx)
     n_elec_dwn=n_elec_dwn+rho_down(idx) 
  end do
   
  n_elec_up=n_elec_up*(omega/dfftp%nnr)
  n_elec_dwn=n_elec_dwn*(omega/dfftp%nnr)
   
  write(stdout,'(/ / A)')       "-----------------------------------------------"
  write(stdout,'(A, F15.8)')    "     Number of spin up electrons = ",n_elec_up
  write(stdout,'(A /)')         "-----------------------------------------------"
  
  write(stdout,'(/ / A)')       "-----------------------------------------------"
  write(stdout,'(A, F15.8)')    "     Number of spin down electrons = ",n_elec_dwn
  write(stdout,'(A /)')         "-----------------------------------------------"
  ...
  !write(stdout,*)'vdW_xc_energy', vdW_xc_energy
...
!DC added
  ...
  SUBROUTINE vdW_DF_energy_3ord (thetas_3ord, vdW_xc_energy_3ord)
  ...
  COMPLEX(DP), INTENT(INOUT) :: thetas_3ord(:,:)       ! On input this variable holds the theta
                                                       ! functions (equation 8, SOLER) in the format
                                                       ! thetas(grid_point, theta_i). On output
                                                       ! this array holds u_alpha(k) =
                                                       ! Sum_j[theta_beta(k)phi_alpha_beta(k)].

  REAL(DP), INTENT(OUT) :: vdW_xc_energy_3ord          ! The non-local correlation energy.

  REAL(DP), ALLOCATABLE :: kernel_of_k_3ord(:,:,:)     ! This array will hold the interpolated kernel
                                                       ! values for each pair of q values in the q_mesh.
  ...
  INTEGER     :: g_i, q1_i, q2_i, q3_i, i_grid         ! Index variables.

  COMPLEX(DP) :: theta(Nqs_3ord), thetam(Nqs_3ord), theta_g(Nqs_3ord) 
                                                       ! Temporary storage arrays used since we
                                                       ! are overwriting the thetas array here.
!DC added
  COMPLEX(DP) :: theta_gamma(Nqs_3ord),theta_beta(Nqs_3ord)

  REAL(DP)    :: G0_term, G_multiplier

  COMPLEX(DP), ALLOCATABLE :: u_vdw_3ord(:,:)        ! Temporary array holding u_alpha(k).


  !write(stdout,*) 'I am at the begining of energy_3ord'


  ALLOCATE ( u_vdW_3ord(dfftp%nnr,Nqs_3ord), kernel_of_k_3ord(Nqs_3ord, Nqs_3ord, Nqs_3ord) )
  vdW_xc_energy_3ord = 0.0D0
  u_vdW_3ord(:,:)    = CMPLX(0.0_DP, 0.0_DP, kind=dp)
  ...
  DO g_i = 1, ngm
     ...
     IF ( igtongl(g_i) .NE. last_g) THEN
        g = SQRT(gl(igtongl(g_i))) * tpiba
        CALL interpolate_kernel_3ord (g, kernel_of_k_3ord)
        last_g = igtongl(g_i)
     END IF

     theta_beta = thetas_3ord(dfftp%nl(g_i),:)
     theta_gamma = thetas_3ord(dfftp%nl(g_i),:)

     DO q3_i = 1, Nqs_3ord
      DO q2_i = 1, Nqs_3ord
       DO q1_i = 1, Nqs_3ord
           u_vdW_3ord(dfftp%nl(g_i),q3_i)  = u_vdW_3ord(dfftp%nl(g_i),q3_i) + &
                kernel_of_k_3ord(q3_i,q2_i,q1_i)*theta_beta(q2_i)*theta_gamma(q1_i)
       END DO   
      END DO
      vdW_xc_energy_3ord = vdW_xc_energy_3ord + G_multiplier * (u_vdW_3ord(dfftp%nl(g_i),q3_i)* &    
                             CONJG(theta_beta(q3_i))*CONJG(theta_gamma(q3_i)))
     END DO   

     IF (g_i < gstart ) vdW_xc_energy_3ord = vdW_xc_energy_3ord / G_multiplier

  END DO

  IF ( gamma_only ) u_vdW_3ord(dfftp%nlm(:),:) = CONJG( u_vdW_3ord(dfftp%nl(:),:) )
  ...
  vdW_xc_energy_3ord = 0.5D0 * e2 * omega * omega * vdW_xc_energy_3ord

  !write(stdout,*) 'vdW_xc_energy_3ord', vdW_xc_energy_3ord  

  thetas_3ord(:,:) = u_vdW_3ord(:,:)
  DEALLOCATE ( u_vdW_3ord, kernel_of_k_3ord )

  END SUBROUTINE vdW_DF_energy_3ord
  ...
  !write(stdout,*) 'potential', potential
...
!DC added
  ...
  SUBROUTINE vdW_DF_potential_3ord (q0, dq0_drho, dq0_dgradrho, grad_rho, u_vdW_3ord, potential_3ord)
  ...
  COMPLEX(DP), INTENT(IN) :: u_vdW_3ord(:,:)               ! The functions u_alpha(r) obtained by
                                                      ! inverse transforming the functions
                                                      ! u_alph(k). See equations 11 and 12 in SOLER.

  REAL(DP), INTENT(INOUT) :: potential_3ord(:)        ! The non-local correlation potential for
                                                      ! points on the grid over the whole cell (not
                                                      ! just those assigned to this processor).

  REAL(DP), ALLOCATABLE, SAVE :: d2y_dx2(:,:)         ! Second derivatives of P_beta polynomials
                                                      ! for interpolation.

  INTEGER :: i_grid, P_i, Q_i, icar                        ! Index variables.
  ...
  REAL(DP) :: y(Nqs_3ord), dP_dq0, P, Q               ! The y values for a given polynomial (all 0
                                                      ! exept for element i of P_i) The derivative
                                                      ! of P at a given q0 and the value of P at a
                                                      ! given q0. Both of these are interpolated
                                                      ! below.
  ...
  !write(stdout,*) 'I am at the begining of potential_3ord'


  ALLOCATE ( h_prefactor(dfftp%nnr), h(dfftp%nnr) )

  potential_3ord     = 0.0D0
  h_prefactor   = 0.0D0
  ...
  IF (.NOT. ALLOCATED( d2y_dx2) ) THEN
     ...
     ALLOCATE( d2y_dx2(Nqs_3ord, Nqs_3ord) )
     CALL initialize_spline_interpolation ( q_mesh_3ord, d2y_dx2(:,:) )
  ...
  DO i_grid = 1, dfftp%nnr
     ...
     DO WHILE ( (q_hi - q_low) > 1)
        ...
        IF (q_mesh_3ord(q) > q0(i_grid)) THEN
           q_hi = q
        ELSE
           q_low = q
        END IF
     ...
     IF ( q_hi == q_low ) CALL errore('vdW_DF_potential_3rd_ord','qhi == qlow',1)

     dq = q_mesh_3ord(q_hi) - q_mesh_3ord(q_low)

     a = (q_mesh_3ord(q_hi) - q0(i_grid))/dq
     b = (q0(i_grid) - q_mesh_3ord(q_low))/dq
     c = (a**3 - a)*dq**2/6.0D0
     d = (b**3 - b)*dq**2/6.0D0
     e = (3.0D0*a**2 - 1.0D0)*dq/6.0D0
     f = (3.0D0*b**2 - 1.0D0)*dq/6.0D0
     ...
     DO Q_i = 1, Nqs_3ord
        ...
        Q      = a*y(q_low) + b*y(q_hi)  + c*d2y_dx2(Q_i,q_low) + d*d2y_dx2(Q_i,q_hi)
        dQ_dq0 = (y(q_hi) - y(q_low))/dq - e*d2y_dx2(Q_i,q_low) + f*d2y_dx2(Q_i,q_hi)
        ! --------------------------------------------------------------
        ! The first term in equation 10 of SOLER.
        potential_3ord(i_grid) = potential_3ord(i_grid) + u_vdW_3ord(i_grid,P_i)* ( P + dP_dq0 * dq0_drho(i_grid) )  
        potential_3ord(i_grid) = potential_3ord(i_grid) + u_vdW_3ord(i_grid,Q_i)* ( Q + dQ_dq0 * dq0_drho(i_grid) )  
        IF (q0(i_grid) .NE. q_mesh_3ord(Nqs_3ord)) THEN
           h_prefactor(i_grid) = h_prefactor(i_grid) + u_vdW_3ord(i_grid,P_i)* ( dP_dq0 * dq0_dgradrho(i_grid) ) 
           h_prefactor(i_grid) = h_prefactor(i_grid) + u_vdW_3ord(i_grid,Q_i)* ( dQ_dq0 * dq0_dgradrho(i_grid) ) 
        END IF
     END DO
     END DO
  ...
  DO icar = 1,3
     ...
     IF (gamma_only) h(dfftp%nlm(:)) = CONJG(h(dfftp%nl(:)))
     CALL invfft ('Rho', h, dfftp)
     potential_3ord(:) = potential_3ord(:) - REAL(h(:))

  END DO

  !write(stdout,*) 'potential_3ord', potential_3ord

  DEALLOCATE ( h_prefactor, h )

  END SUBROUTINE vdW_DF_potential_3ord
...
!DC added
  ...
  SUBROUTINE interpolate_kernel_3ord (k, kernel_of_k_3ord)
  ...
  REAL(DP), INTENT(INOUT) :: kernel_of_k_3ord(:,:,:)  ! An output array (allocated outside this routine)
                                                      ! that holds the interpolated value of the kernel
                                                      ! for each sets of three q points 
                                                      ! (i.e. the phi_alpha_beta_gamma)
                                                      ! of the Soler method.

  INTEGER  :: q1_i, q2_i, q3_i, k_i                  ! Indexing variables.

  REAL(DP) :: A, B, C, D                       ! Intermediate values for the interpolation.



  !write(stdout,*) 'I am at the begining of interpolate_kernel_3ord'
     ...
     DO q1_i = 1, Nqs_3ord
        DO q2_i = 1, q1_i
          DO q3_i = 1, q2_i
           ...
           kernel_of_k_3ord(q1_i, q2_i, q3_i) = kernel3ord(k_i,q1_i, q2_i,q3_i)
           kernel_of_k_3ord(q1_i, q3_i, q2_i) = kernel3ord(k_i,q1_i, q3_i,q2_i)
           kernel_of_k_3ord(q2_i, q1_i, q3_i) = kernel3ord(k_i,q2_i, q1_i,q3_i)
           kernel_of_k_3ord(q2_i, q3_i, q1_i) = kernel3ord(k_i,q2_i, q3_i,q1_i)
           kernel_of_k_3ord(q3_i, q1_i, q2_i) = kernel3ord(k_i,q3_i, q1_i,q2_i)
           kernel_of_k_3ord(q3_i, q2_i, q1_i) = kernel3ord(k_i,q3_i, q2_i,q1_i)
  ...
  DO q1_i = 1, Nqs_3ord
     DO q2_i = 1, q1_i
       DO q3_i = 1, q2_i
        ...
        kernel_of_k_3ord(q1_i, q2_i, q3_i) = A*kernel3ord(k_i, q1_i, q2_i, q3_i) + B*kernel3ord(k_i+1, q1_i, q2_i, q3_i) &
                        + (C*d2phi3ord_dk2(k_i, q1_i, q2_i, q3_i) + D*d2phi3ord_dk2(k_i+1, q1_i, q2_i, q3_i))

        kernel_of_k_3ord(q1_i, q3_i, q2_i) = kernel_of_k_3ord(q1_i, q2_i, q3_i)
        kernel_of_k_3ord(q2_i, q1_i, q3_i) = kernel_of_k_3ord(q1_i, q2_i, q3_i)
        kernel_of_k_3ord(q2_i, q3_i, q1_i) = kernel_of_k_3ord(q1_i, q2_i, q3_i)
        kernel_of_k_3ord(q3_i, q1_i, q2_i) = kernel_of_k_3ord(q1_i, q2_i, q3_i)
        kernel_of_k_3ord(q3_i, q2_i, q1_i) = kernel_of_k_3ord(q1_i, q2_i, q3_i)

       END DO
     END DO
  END DO
 
  WRITE(stdout,*) kernel_of_k_3ord
   
  END SUBROUTINE interpolate_kernel_3ord
  ...
  COMPLEX(DP), ALLOCATABLE :: thetas(:,:),thetas_3ord(:,:) ! Thetas.
  INTEGER                  :: i_proc, theta_i, l, m

  REAL(DP)                 :: sigma_grad(3,3)
  REAL(DP)                 :: sigma_ker(3,3)

  REAL(DP)                 :: sigma_grad_3ord(3,3)
  REAL(DP)                 :: sigma_ker_3ord(3,3)
  ...
  ALLOCATE( total_rho(dfftp%nnr), grad_rho(3,dfftp%nnr), thetas(dfftp%nnr, Nqs), &
            thetas_3ord(dfftp%nnr,Nqs_3ord), q0(dfftp%nnr), dq0_drho(dfftp%nnr), &
            dq0_dgradrho(dfftp%nnr) )
...
!DC added
  ...
  sigma_grad_3ord(:,:) = 0.0_DP
  sigma_ker_3ord(:,:)  = 0.0_DP

  CALL get_q0_on_grid (total_rho, grad_rho, q0, dq0_drho, dq0_dgradrho, thetas_3ord)
  CALL vdW_DF_stress3ord_gradient (total_rho, grad_rho, q0, dq0_drho, dq0_dgradrho, thetas_3ord, sigma_grad_3ord)
  CALL vdW_DF_stress3ord_kernel   (total_rho, q0, thetas_3ord, sigma_ker_3ord)

  sigma = - (sigma_grad_3ord + sigma_ker_3ord) + sigma
  ...
  DEALLOCATE( total_rho, grad_rho, thetas, thetas_3ord, q0, dq0_drho, dq0_dgradrho )
...
!DC added
  ...
  ! ####################################################################
  !                     |                              |
  !                     |  VDW_DF_STRESS3ord_GRADIENT  |
  !                     |______________________________|

  SUBROUTINE vdW_DF_stress3ord_gradient (total_rho, grad_rho, q0, &
             dq0_drho, dq0_dgradrho, thetas, sigma)
  ...
  COMPLEX(DP), ALLOCATABLE :: u_vdW_3ord(:,:)        !

  REAL(DP), ALLOCATABLE    :: d2y_dx2(:,:)           !
  REAL(DP) :: y(Nqs_3ord), dP_dq0, P, a, b, c, d, e, f    ! Interpolation.
  REAL(DP) :: dq                                     !
  ...
  ALLOCATE( d2y_dx2(Nqs_3ord, Nqs_3ord))
  ALLOCATE( u_vdW_3ord(dfftp%nnr, Nqs_3ord))
  ...
  CALL thetas3ord_to_uk3ord(thetas, u_vdW_3ord)
  ...
  DO theta_i = 1, Nqs_3ord
     CALL invfft('Rho', u_vdW_3ord(:,theta_i), dfftp)
  END DO
  ...
  DO i_grid = 1, dfftp%nnr
     ...
     DO q_i = 1, Nqs_3ord
        ...
        prefactor = u_vdW_3ord(i_grid,q_i) * dP_dq0 * dq0_dgradrho(i_grid) / grad2
  ...
  DEALLOCATE( d2y_dx2,  u_vdW_3ord )

  END SUBROUTINE vdW_DF_stress3ord_gradient
...
!DC added 
  ...
  ! ####################################################################
  !                      |                            |
  !                      |  VDW_DF_STRESS3ord_KERNEL  |
  !                      |____________________________|

  SUBROUTINE vdW_DF_stress3ord_kernel (total_rho, q0, thetas, sigma)
  ...
  REAL(DP), ALLOCATABLE   :: dkernel3ord_of_dk(:,:,:)

  INTEGER                 :: l, m, q1_i, q2_i , q3_i, g_i
  INTEGER                 :: last_g, theta_i
  REAL(DP)                :: g2, ngmod2, g_kernel, G_multiplier
  ...
  ALLOCATE( dkernel3ord_of_dk(Nqs_3ord, Nqs_3ord, Nqs_3ord) )
  ...
  DO g_i = gstart, ngm
     ...
     IF ( igtongl(g_i) .NE. last_g) THEN
        ...
        CALL interpolate_Dkernel3ord_Dk(g_kernel, dkernel3ord_of_dk)  ! Gets the derivatives.
        last_g = igtongl(g_i)
       ...
       DO q2_i = 1, Nqs_3ord
         DO q1_i = 1, Nqs_3ord
           DO l = 1, 3
              ...
              sigma (l, m) = sigma (l, m) - G_multiplier * 0.5 * e2 * thetas(dfftp%nl(g_i),q1_i) * &
                             thetas(dfftp%nl(g_i),q1_i) * dkernel3ord_of_dk(q1_i,q2_i,q3_i) * &
                             conjg(thetas(dfftp%nl(g_i),q2_i)) * conjg(thetas(dfftp%nl(g_i),q3_i)) * &
                             (g (l, g_i) * g (m, g_i) * tpiba2) / g_kernel
              END DO
           END DO
        END DO
  ...
  DEALLOCATE( dkernel3ord_of_dk )

  END SUBROUTINE vdW_DF_stress3ord_kernel
...
!DC added
  ! ####################################################################
  !                        |                             |
  !                        | INTERPOLATE_DKERNEL_DK_3ord |
  !                        |_____________________________|

  SUBROUTINE interpolate_Dkernel3ord_Dk (k, dkernel3ord_of_dk)
  ...
  REAL(DP), INTENT(INOUT) :: dkernel3ord_of_dk(Nqs_3ord,Nqs_3ord,Nqs_3ord) ! An output array (allocated outside this
                                                    ! routine) that holds the interpolated value of
                                                    ! the kernel for each pair of q points (i.e. the
                                                    ! phi_alpha_beta of the Soler method.
  ...
  !write(stdout,*) 'I am at the begining of interpolate_dkernel3ord_dk'
  ...
  DO q1_i = 1, Nqs_3ord
     DO q2_i = 1, q1_i
       DO q3_i = 1, q2_i
        ...
        dkernel3ord_of_dk(q1_i, q2_i, q3_i) = dAdk*kernel3ord(k_i, q1_i, q2_i, q3_i) + &
                              dBdk*kernel3ord(k_i+1, q1_i, q2_i, q3_i)  +              &
                              dCdk*d2phi3ord_dk2(k_i, q1_i, q2_i, q3_i) +              &
                              dDdk*d2phi3ord_dk2(k_i+1, q1_i, q2_i, q3_i)

        dkernel3ord_of_dk(q1_i, q3_i, q2_i) = dkernel3ord_of_dk(q1_i, q2_i, q3_i)
        dkernel3ord_of_dk(q2_i, q1_i, q3_i) = dkernel3ord_of_dk(q1_i, q2_i, q3_i)
        dkernel3ord_of_dk(q2_i, q3_i, q1_i) = dkernel3ord_of_dk(q1_i, q2_i, q3_i)
        dkernel3ord_of_dk(q3_i, q1_i, q2_i) = dkernel3ord_of_dk(q1_i, q2_i, q3_i)
        dkernel3ord_of_dk(q3_i, q2_i, q1_i) = dkernel3ord_of_dk(q1_i, q2_i, q3_i)
       END DO
     END DO
  END DO

  END SUBROUTINE interpolate_Dkernel3ord_Dk
...
!DC added
  ! ####################################################################
  !                          |                      |
  !                          | thetas3ord_to_uk3ord |
  !                          |______________________|


  SUBROUTINE thetas3ord_to_uk3ord (thetas, u_vdW_3ord)
  ...
  COMPLEX(DP), INTENT(IN)  :: thetas(:,:)  ! On input this variable holds the theta functions
                                                ...
                                                ! thetas(grid_point, theta_i,
                                                ! theta_j).
  COMPLEX(DP), INTENT(OUT) :: u_vdW_3ord(:,:)   ! On output this array holds u_alpha(k) =
                                                ! Sum_j[theta_beta(k)Sum_n[theta_gamma(k)
                                                ! phi_alpha_beta_gamma(k)].

  REAL(DP), ALLOCATABLE    :: kernel_of_k_3ord(:,:,:)  ! This array will hold the interpolated kernel
                                                       ! values for any three set of q values in 
                                                       ! the q_mesh.

  REAL(DP) :: g
  INTEGER  :: last_g, g_i, q1_i, q2_i, q3_i, i_grid   ! Index variables.


  COMPLEX(DP) :: theta_3ord_beta(Nqs_3ord)          ! Temporary storage vector used since we are
  COMPLEX(DP) :: theta_3ord_gamma(Nqs_3ord)          ! overwriting the thetas array here.

  !write(stdout,*) 'I am at the begining of thetas3ord_to_uk3ord'



  ALLOCATE( kernel_of_k_3ord(Nqs_3ord, Nqs_3ord, Nqs_3ord) )
  ...
  DO g_i = 1, ngm
     ...
     theta_3ord_beta = thetas(dfftp%nl(g_i),:)
     theta_3ord_gamma = thetas(dfftp%nl(g_i),:)
     ...
     DO q3_i = 1, Nqs_3ord
      DO q2_i = 1, Nqs_3ord
       DO q1_i = 1, Nqs_3ord
           u_vdW_3ord(dfftp%nl(g_i),q3_i) = u_vdW_3ord(dfftp%nl(g_i),q3_i) + &
                 kernel_of_k_3ord(q3_i,q2_i,q1_i)*theta_3ord_gamma(q2_i)*theta_3ord_beta(q1_i)
  ...
  DEALLOCATE( kernel_of_k_3ord )

  END SUBROUTINE thetas3ord_to_uk3ord
...
!DC added
  ! ####################################################################
  !                           |                           |
  !                           | GENERATE_KERNEL_3rd_ORDER |
  !                           |___________________________|
  !
  !  DC 01/14/2020 : This is an attempt to code the third order expansion 
  !                  of van-der Waals term

  SUBROUTINE generate_kernel_3rd_order

  IMPLICIT NONE

  INTEGER  :: a_i, b_i, c_i, q1_i, q2_i, q3_i, r_i, p , r, x1, x2
  ! Indexing variables.
  ...
  REAL(DP) :: sin_a( Nintegration_points ), cos_a( Nintegration_points ), &
              cot_a( Nintegration_points ) 
  ! Sine and cosine values of the aforementioned points a.
  
  !DC : 01/14/2020
  REAL(DP) :: w_0(Nintegration_points), w_1(Nintegration_points), &
              w_2(Nintegration_points)

  REAL(DP) :: d1, d2, d3, d, integral
  REAL(DP) :: d121, d122, d231, d232, d311, d312
  ! Intermediate values.
  ...
  REAL(DP), ALLOCATABLE :: phi3ord(:,:), phi_deriv3ord_k(:,:)
  ! Arrays to store the kernel functions and their second derivatives.
  ! They are stored as phi(radial_point, idx).

  INTEGER, ALLOCATABLE  :: indices3ord(:,:), proc_indices3ord(:,:)
  ...
  CALL start_clock ( 'vdW_kernel_3rd_order' )

 ! write(stdout,*) 'I am at the begining of generate_kernel_3rd_order'

  ! --------------------------------------------------------------------
  ! The total number of phi_alpha_beta functions that have to be
  ! calculated.

! DC 01/14/2020 : Nqs is the number of gridpoint from Soler's equation and 
!                 it needs to be changed.
 
!  x1 = CEILING(LOG(REAL(Nqs))/LOG(2.0))
!  write(stdout,*) 'x1',x1
!  Ntotal = Nqs*(Nqs+1)*x1/2
!  write(stdout,*) 'Ntotal',Ntotal
!  ALLOCATE ( indices3ord(Ntotal, 3) )

  Ntotal = Nqs_3ord*(Nqs_3ord+1)*(Nqs_3ord+2)/6
  !write(stdout,*) 'Ntotal',Ntotal
  ALLOCATE ( indices3ord(Ntotal, 3) )
...
!        write(stdout,*) 'r', r
!      DO q3_i = 1, r, q3_i*2
!        write(stdout,*) 'q3_i',q3_i
!        indices3ord(idx,1)=q1_i
!        indices3ord(idx,2)=q2_i 
!        indices3ord(idx,3)=q3_i
!        write(stdout,*) indices3ord(idx,1),indices3ord(idx,2),indices3ord(idx,3)
!        idx=idx+1
!      END DO
!    END DO
!  END DO 
 ...
 ! write(stdout,*) 'Ntotal',Ntotal,'idx', idx
  ...
  Nper   = Ntotal/nproc
  Nextra = MOD(Ntotal, nproc)
  !write(stdout,*) 'nproc', nproc, 'mpime', mpime, 'Nper', Nper, 'Nextra', Nextra

  ALLOCATE( proc_indices3ord(nproc, 2) )
  ...
  DO proc_i = 1, nproc
     ...
     ! This prevent trouble if number of processors exceeds Ntotal.
     IF ( proc_i > Ntotal ) THEN
        start_q    = Ntotal
        end_q      = Ntotal
     END IF
     ...
     proc_indices3ord(proc_i, 1) = start_q
     proc_indices3ord(proc_i, 2) = end_q
  ...
  my_Nqs    = my_end_q - my_start_q + 1
  ALLOCATE( phi3ord( 0:Nr_points, my_Nqs), phi_deriv3ord_k( 0:Nr_points, my_Nqs) )

  phi3ord         = 0.0D0
  phi_deriv3ord_k = 0.0D0
  kernel3ord      = 0.0D0
  d2phi3ord_dk2   = 0.0D0
  ...
  DO a_i=1, Nintegration_points
     ...
     weights(a_i)   = weights(a_i)*( 1 + a_points2(a_i) )
     sin_a(a_i)     = SIN( a_points(a_i) )
     cot_a(a_i)     = sin_a(a_i)/COS( a_points(a_i) )
     w_0(a_i)       = sin_a(a_i)/a_points(a_i)
     w_1(a_i)       = (1.0d0/(a_points2(a_i))) -             &
                      (cot_a(a_i)/(a_points(a_i)))
  END DO


  !----------------------------------------------------------------------
  ! DC 01/14/2020 : Adding the angular terms for 3rd order expansion


  
  DO a_i=1, Nintegration_points
  DO b_i=1, Nintegration_points 
  DO c_i=1, Nintegration_points 
      G1_abc(a_i, b_i, c_i) = weights(a_i)*weights(b_i)*                   &
            weights(c_i)* w_0(a_i)*w_0(b_i)*w_0(c_i)
      G2_abc(a_i, b_i, c_i) = weights(a_i)*weights(b_i)*                   &
            weights(c_i)*w_0(a_i)*w_0(b_i)*w_0(c_i)*                       &
            ((1.0D0 - 2.0D0*w_1(b_i))*(1.0D0 - 2.0D0*w_1(c_i))+            &
            2.0d0*w_1(b_i)*w_1(c_i))
      G3_abc(a_i, b_i, c_i) = weights(a_i)*weights(b_i)*                   &
            weights(c_i)*w_0(a_i)*w_0(b_i)*w_0(c_i)*                       &
            (2.0D0*w_1(a_i)*2.0D0*w_1(b_i)*2.0D0*w_1(c_i)-                 &
             2.0D0*(1.0d0 - 2.0d0*w_1(a_i))*(1.0d0 - 2.0d0*w_1(b_i))*      &
            (1.0d0 - 2.0d0*w_1(c_i)))
  END DO
  END DO
  END DO

!  write(stdout,*) 'I am at the middle of generate_kernel_3rd_order'


  ! --------------------------------------------------------------------
  ! Now, we loop over all the three subset (q1,q2,q3) that are assigned to us and
  ! perform our calculations.

  DO idx = 1, my_Nqs
     
!  write(stdout,*) 'idx', idx, 'my_Nqs', my_Nqs
    
     ! -----------------------------------------------------------------
     ! First, get the value of phi(q1*r, q2*r, q3*r) for each r and the
     ! particular values of q1 q2 and q3 we are using.

     DO r_i = 1, Nr_points
        d1  = (q_mesh_3ord( indices3ord(idx+my_start_q-1, 1) ) * dr * r_i) * (2.0D0/3.0D0)
        d2  = (q_mesh_3ord( indices3ord(idx+my_start_q-1, 2) ) * dr * r_i) * (2.0D0/3.0D0)
        d3  = (q_mesh_3ord( indices3ord(idx+my_start_q-1, 3) ) * dr * r_i) * (2.0D0/3.0D0)
        phi3ord(r_i, idx) =phi_value3ord(d1,d2,d3) 
        !write(stdout,*) 'ri' ,r_i, 'phi_value_3ord', phi3ord(r_i,idx)
     END DO

  !   write(stdout,*) 'idx', idx, 'phi3ord', phi3ord(:,idx)

     ! -----------------------------------------------------------------
     ! Now, perform a radial FFT to turn our phi_alpha_beta(r) into
     ! phi_alpha_beta(k) needed for SOLER equation 8.

     CALL radial_fft( phi3ord(:,idx) )
     ...
     CALL set_up_splines( phi3ord(:, idx), phi_deriv3ord_k(:, idx))
  ...
  DO proc_i = 0, nproc-1
     ...
     CALL mp_get ( phi3ord      , phi3ord      , mpime, 0, proc_i, 0, intra_image_comm )
     CALL mp_get ( phi_deriv3ord_k, phi_deriv3ord_k, mpime, 0, proc_i, 0, intra_image_comm )

     IF ( mpime == 0 ) THEN

       DO idx = proc_indices3ord(proc_i+1,1), proc_indices3ord(proc_i+1,2)
   !        write(stdout,*) 'idx',idx,'I am entering into processing the 3rd order kernel'
           q1_i = indices3ord(idx, 1)
           q2_i = indices3ord(idx, 2)
           q3_i = indices3ord(idx, 3)
           kernel3ord (:, q1_i, q2_i, q3_i) = phi3ord (:, idx - proc_indices3ord(proc_i+1,1) +1)
           d2phi3ord_dk2 (:, q1_i, q2_i, q3_i) = phi_deriv3ord_k (:, idx - proc_indices3ord(proc_i+1,1) +1)
           kernel3ord (:, q1_i, q3_i, q2_i) = kernel3ord (:, q1_i, q2_i, q3_i)
           d2phi3ord_dk2 (:, q1_i, q3_i, q2_i) = d2phi3ord_dk2 (:, q1_i, q2_i, q3_i)
           kernel3ord (:, q2_i, q1_i, q3_i) = kernel3ord (:, q1_i, q2_i, q3_i)
           d2phi3ord_dk2 (:, q2_i, q1_i, q3_i) = d2phi3ord_dk2 (:, q1_i, q2_i, q3_i)
           kernel3ord (:, q2_i, q3_i, q1_i) = kernel3ord (:, q1_i, q2_i, q3_i)
           d2phi3ord_dk2 (:, q2_i, q3_i, q1_i) = d2phi3ord_dk2 (:, q1_i, q2_i, q3_i)
           kernel3ord (:, q3_i, q1_i, q2_i) = kernel3ord (:, q1_i, q2_i, q3_i)
           d2phi3ord_dk2 (:, q3_i, q1_i, q2_i) = d2phi3ord_dk2 (:, q1_i, q2_i, q3_i)
           kernel3ord (:, q3_i, q2_i, q1_i) = kernel3ord (:, q1_i, q2_i, q3_i)
           d2phi3ord_dk2 (:, q3_i, q2_i, q1_i) = d2phi3ord_dk2 (:, q1_i, q2_i, q3_i)
    !       write(stdout,*) 'I am leaving the 3rd order kernel' 
    !       write(stdout,*) idx, q1_i, q2_i, q3_i, kernel3ord(:,q1_i,q2_i,q3_i), d2phi3ord_dk2(:,q1_i,q2_i,q3_i)
       END DO
  ...
  CALL mp_bcast ( kernel3ord   , 0, intra_image_comm )
  CALL mp_bcast ( d2phi3ord_dk2, 0, intra_image_comm )

 
  DEALLOCATE( indices3ord, proc_indices3ord, phi3ord, phi_deriv3ord_k )

  !write(stdout,*) 'I am leaving generate_kernel_3rd_order'

  ! --------------------------------------------------------------------
  ! Stop the timer.

  CALL stop_clock ( 'vdW_kernel_3rd_order' )

  END SUBROUTINE generate_kernel_3rd_order
...
!DC added
  ...
  !                            |_________________|
  !
  ! This function returns the value of the 3 point kernel as calculated.

  REAL(DP) FUNCTION phi_value3ord(d1, d2, d3)

  REAL(DP), INTENT(IN) :: d1, d2, d3
  ! The point at which to evaluate the kernel. d1 = 2*q1*r/3, d2 = 2*q2*r/3, d3 = 2*q3*r/3
  ...
  REAL(DP) :: w, x, y, z, p , q, T3
  ! Intermediate values.

  REAL(DP) :: nu(Nintegration_points), nu1(Nintegration_points), &
              nu2(Nintegration_points)
 ...
 ! write(stdout,*) 'I am entering phi_value_3rd_order'



  phi_value3ord  = 0.0D0
  IF ( d1==0.0D0 .AND. d2==0.0D0 .AND. d3==0.0D0 ) THEN
     phi_value3ord  = 0.0D0
     RETURN
  END IF
  ...
  DO a_i = 1, Nintegration_points
     ...
     IF ( a_points(a_i) <= small .AND. d1 > small) THEN
        nu2(a_i) = 9.0D0/8.0D0*d3**2/pi
     ELSE IF (d1 <= small) THEN
        nu2(a_i) = a_points(a_i)**2/2.0D0
     ELSE
        nu2(a_i) = a_points(a_i)**2/((-EXP(-(a_points(a_i)**2*gamma)/d3**2) + 1.0D0)*2.0D0)
     END IF
     
  !   write(stdout,*) a_i, nu(a_i),nu1(a_i),nu2(a_i)

  END DO

  !write(stdout,*) 'I am at the middle of phi_value_3rd_order'
  ...
  DO c_i = 1, Nintegration_points
     ...
     !write(stdout,*) 'w', w, 'x', x, 'y', y, 'z', z, 'p', p, 'q', q 

     ! --------------------------------------------------------------
     ! Again, watch out for possible numerical problems

     IF (w < small .or. x<small .OR. y<small .OR. z<small .OR. p<small .OR. q<small) CYCLE
     T3= 1.0D0/((x+y)*(w+z)*(p+q))*(((x+w+p)/((p+w)*(x+w)*(x+p))) + ((x+w+q)/((q+w)*(x+w)*(x+q))) + &
                                   ((x+z+p)/((z+p)*(x+z)*(x+p))) + ((x+z+q)/((q+x)*(x+z)*(z+q))) +  &
                                   ((y+w+p)/((y+p)*(y+w)*(w+p))) + ((y+w+q)/((q+w)*(y+w)*(y+q))) +  &
                                   ((y+z+p)/((y+p)*(y+z)*(z+p))) + ((y+z+q)/((q+y)*(y+z)*(z+q))) )
     !write(stdout,*) 'T3', T3, 'G1_abc', G1_abc(a_i, b_i, c_i), 'G2_abc', G2_abc(a_i,b_i,c_i), 'G3_abc',G3_abc(a_i,b_i,c_i) 
     phi_value3ord = phi_value3ord + T3 *(( 1.0D0/6.0D0) * G1_abc(a_i, b_i, c_i)- &
                      (1.0D0/2.0D0) * G2_abc(a_i,b_i,c_i) + (1.0D0/3.0D0) *  &
                      G3_abc(a_i,b_i,c_i))
    ! write(stdout,*) 'phi_value_3ord', phi_value3ord 
  END DO
  END DO
  END DO

  phi_value3ord = 1.0D0/pi**2*phi_value3ord

  !write(stdout,*) 'phi_3rd_order', phi_value3ord
  !write(stdout,*) 'I am leaving phi_value_3rd_order'


  END FUNCTION phi_value3ord
  ...
  !write(stdout,*) 'I am leaving radial_fft'
  ...
  !write(stdout,*) 'I am leaving set_up_splines'
  ...
  IF ( iverbosity > 0 ) THEN
     WRITE(stdout,'(5x,"Carrying out 3rd order vdW-DF run using the following parameters:")')
     WRITE(stdout,'(5X,A,I3,A,I5,A,F8.3)' ) "Nqs    = ", Nqs_3ord, "  Npoints = ", Nr_points, &
                  "  r_max = ", r_max
     WRITE(stdout,'(5X,"q_mesh =",4F12.8)') (q_mesh_3ord(idx), idx=1, 4)
     WRITE(stdout,'(13X,4F12.8)') (q_mesh_3ord(idx), idx=5, Nqs_3ord)
  END IF
