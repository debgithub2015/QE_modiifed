...
subroutine b86b (rho, grho, iflag, sx, v1x, v2x)
  ...
  !
!! DC added
  ! iflag=4: I. Hamada, Phys. Rev. B 89, 121103(R) (B86R), fitted 
  !          for binding energy of s66, with minimum error in energy 
  !          at the far separation.
  !
  USE kinds, ONLY : DP
  USE constants, ONLY : pi
  implicit none
...
!  real(DP) :: k (3), mu(3)
  real(DP) :: k (4), mu(4)
  data k / 0.5757d0, 1.0000d0, 0.711357d0, 0.58d0 /, &
       mu/ 0.2449d0, 0.1234d0, 0.1234d0, 0.1234d0 /
  !
  agrho = sqrt (grho)
  kf = c2 * rho**third
  dsg = 0.5d0 / kf
...
!  dfx1 = 1.d0 + (1.d0 - c6)*mu(iflag)*s2/k(iflag)
...
subroutine pbex (rho, grho, iflag, sx, v1x, v2x)
  ...
  ! 
  ! iflag=8  Klimes, Bowler, Michaelides, PRB 83, 195131 (2011)
  !                       beta=mu_PBEsol/1.70 
  USE kinds, ONLY : DP
  USE constants, ONLY : pi
#if defined(__LIBXC)
  ...
  if (iflag.eq.8)  call errore('pbex','case not implemented with libxc',iflag)
  ...
  real(DP), parameter :: third = 1._DP / 3._DP, c1 = 0.75_DP / pi , &
       ...
       c6 = c2*2.51984210, c7=5._DP/6._DP, c8=0.8_DP, c9=1.645_DP !c9=2.0_DP ! (3pi^2)^(1/3)*2^(4/3)
  !DC added/changed
  ! parameters of the functional
  real(DP) :: k (8), mu(8), ev(6)
  !           pbe        rpbe        pbesol   pbeq2d      optB88  optB86b
  !           optb88N
!  data k / 0.804_DP,   1.2450_DP,   0.804_DP , 0.804_DP,  0.0_dp,  0.0_dp, 0.0_dp, 0.0_dp/, &
  data k / 0.804_DP,   1.2450_DP,   0.804_DP , 0.804_DP,  0.0_dp,  0.0_dp, 0.0_dp, 0.0_dp/, &
       mu/ 0.2195149727645171_DP, 0.2195149727645171_DP, 0.12345679012345679_DP, &
!           0.12345679012345679_DP,  0.22_dp, 0.1234_dp, 0.22_DP/, &
           0.12345679012345679_DP,  0.22_dp, 0.1234_dp, 0.0_dp, 0.12345_dp/, & !DC added
