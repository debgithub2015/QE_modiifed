...
MODULE funct
  ...
  USE kinds,       ONLY: DP
!DC added
  USE input_parameters, ONLY: vdw_h_switch !,x_param, optb88_param, b86r_param
#if defined(__LIBXC)
  USE xc_f90_types_m
  USE xc_f90_lib_m
#endif
...
!DC added 20/12/2019 managing the h function
!  PUBLIC  :: get_ihfun
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!
  ...
  ! DC 12/19/2019
  !              "ob8n" revised optb88 exchange          igcx =43
  !                        beta=mu_PBEsol/1.10
  ! DC 12/19/2019
  !              "b86n" revised Becke (b86b)             igcx =44
  !                        kappa=0.58
  !
  ...
  ! DC added 12/20/2019
  ! vdW-DF functional with various switching functions
  !             "dion"   default switching function     ihfun=0 (default) 
  !             "hC6"    C6 switching function          ihfun=1 
  !             "h3"     DF3 switching function         ihfun=2 
  !
  ! References:
  ...
  ! DC 12/19/2019
  !              b86n    I. Hamada, Phys. Rev. B 89, 121103(R) (2014), with
  !                         kappa=0.81664352
  ! DC 12/12/2019
  !              ob8n    Klimes, Bowler, Michaelides, PRB 83, 195131 (2011)
  !                       beta=mu_PBEsol/1.70
  ...
  !
  ! DC 03/03/2020
  !         Add the switching function references here
  !
  ! ------------------------------------------------------------------------
  !
  ...
  !DC added 12/20/2019
  !    ihfun: type of switching function for vdW-DF correlation
...
!  INTEGER :: ihfun  = notset
  ...
  !
!DC changes 03/03/2020
  INTEGER, PARAMETER :: nxc=8, ncc=10, ngcc=12, nmeta=6, ncnl=6
  INTEGER, PARAMETER :: ngcx = 44
!  INTEGER, PARAMETER :: nhfun=3
  CHARACTER(LEN=4) :: exc, corr, gradx, gradc, meta, nonlocc  !, hfunc
  DIMENSION :: exc(0:nxc), corr(0:ncc), gradx(0:ngcx), gradc(0:ngcc), &
               meta(0:nmeta), nonlocc(0:ncnl)   !, hfunc(0:nhfun)
  !
...
! DC added 03/03/2020
  ...
  !
 ! DATA hfunc/  'DION','hC6','h3' /
! DC added 03/04/2020
!   CHARACTER(LEN=25) :: vdw_h_switch = 'none'
...
!DC added 03/03/2020
!Special case of vdW-DF1 with modified optB88
    CASE ('sla+pw+ob8n+vdw1') ! To be named as DF3-opt1, TIMO will
                              ! implement this against his vdW-DF
                              ! reservations
       dft_defined = set_dft_values(1,4,43,0,1,0)
...
!DC added 03/03/2020
! Special case vdW-DF2 with modified B86R
    CASE( 'sla+pw+b86n+vdw2' )   ! To be named as DF3-opt2, TIMO will
                                 ! implement this against his vdW-DF 
                                 ! reservations
      dft_defined = set_dft_values(1,4,44,0,2,0)
...
!  FUNCTION get_ihfun()
!     INTEGER get_ihfun
!     IF ( vdw_h_switch == 'DION' ) ihfun = 0  ! Default
!     IF ( vdw_h_switch == 'hC6'  ) ihfun = 1        
!     IF ( vdw_h_switch == 'h3'   ) ihfun = 2      
!     write(stdout,*) 'vdw_h_switch', vdw_h_switch 
!     get_ihfun = ihfun
!     RETURN
!  END FUNCTION get_ihfun
  !-----------------------------------------------------------------------
...
!DC added 03/03/2020
!       IF ( x_param ) THEN
!        IF ( optb88_param ) THEN
!          IF (iexch==1 .AND. icorr==4 .AND. igcx==43 .AND. igcc==0) THEN
!            shortname = 'VDW-DF-OBK8'
!          END IF 
!        END IF
!       ELSE 
!        IF (iexch==1 .AND. icorr==4 .AND. igcx==23 .AND. igcc==0) THEN
!          shortname = 'VDW-DF-OBK8' 
!        END IF
!       END IF
...
!       IF ( x_param ) THEN
!        IF ( optb88_param ) THEN
!          IF (iexch==1 .AND. icorr==4 .AND. igcx==44 .AND. igcc==0) THEN
!            shortname = 'VDW-DF2-B86R'
!          END IF 
!        END IF
!       ELSE 
!        IF (iexch==1 .AND. icorr==4 .AND. igcx==26 .AND. igcc==0) THEN
!          shortname = 'VDW-DF2-B86R' 
!        END IF
!       END IF
