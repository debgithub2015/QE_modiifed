...
USE control_flags,     ONLY : iverbosity, gamma_only, gamma_hfunc, alpha_hfunc, beta_hfunc
! DC adds 04/03/2020
!USE input_parameters,  ONLY : gamma_hfunc, alpha_hfunc, beta_hfunc
USE input_parameters,  ONLY: vdw_h_switch !,x_param, optb88_param, b86r_param
!USE funct,             ONLY : get_ihfun 
...
PUBLIC  :: xc_vdW_DF, xc_vdW_DF_spin, vdW_DF_stress,                   &
           ...
           get_ihfun, get_gamma, get_alpha, get_beta
...
CONTAINS
  ...
  FUNCTION get_gamma()
        REAL(DP) ::get_gamma
        get_gamma=gamma_hfunc
        RETURN
  END FUNCTION get_gamma

  FUNCTION get_alpha()
        REAL(DP) ::get_alpha
        get_alpha=alpha_hfunc
        RETURN
...
!  SUBROUTINE set_parameters(gamma_in, alpha_in, beta_in, &
!                        gamm_out, alpha_out, beta_out)
!  USE kinds, ONLY:dp
!   
!  IMPLICIT NONE
!        
!  REAL(DP),INTENT(IN)  :: gamma_in, alpha_in, beta_in
!  REAL(DP),INTENT(OUT) :: gamm_out, alpha_out, beta_out
!
!  gamm_out   =  gamma_in
!  alpha_out  =  alpha_in
!  beta_out   =  beta_in
...
!  write(stdout, * ) gamma_hfunc
!  write(stdout, * ) alpha_hfunc
!  write(stdout, * ) beta_hfunc
  ...
  DO a_i = 1, Nintegration_points
   ...
   CASE (1)                      !hfun=1 vdW-DF-C6 switching function  
    
!     CALL set_parameters(gamma_hfunc, alpha_hfunc, beta_hfunc, gamm, alpha, beta)

     gamm  = get_gamma()
     alpha = get_alpha()
     beta  = get_beta()

     A = (beta + alpha * (0.5d0*alpha - gamm))/(1.0D0+gamm-alpha)
     ...
     ELSE
        ...
        hfunc = 1.0d0 - (1.0d0 + ((alpha-gamm)*y**2 + A*y**4)/(A*y**2 + 1.0d0))* &
                exp(-alpha*y**2)

        nu(a_i) = a_points(a_i)**2/(2.0D0*hfunc)
     END IF
   ...
   CASE(2)                       !hfun=2 vdW-DF3 switching function 
...
!     CALL set_parameters(gamma_hfunc, alpha_hfunc, beta_hfunc, gamm, alpha, beta)
     ...
     ELSE
        ...
        hfunc = 1.0d0 - 1.0d0/(1.0d0 + gamm*y**2 + B*y**4 + alpha*y**8)
   ...
   CASE DEFAULT                   !hfun=0 vdW-DF default (DION) switching function      
...
!     CALL set_parameters(gamma_hfunc, alpha_hfunc, beta_hfunc, gamm, alpha, beta)
     ...
     ELSE IF (d1 <= small) THEN
        nu(a_i) = a_points(a_i)**2/2.0D0
     ELSE
        nu(a_i) = a_points(a_i)**2/((-EXP(-(a_points(a_i)**2*gamm)/d1**2)+ &
                  1.0D0)*2.0D0)
     END IF
     ...
     ELSE IF (d2 < small) THEN
        nu1(a_i) = a_points(a_i)**2/2.0D0
     ELSE
        nu1(a_i) = a_points(a_i)**2/((-EXP(-(a_points(a_i)**2*gamm)/d2**2)+ &
                   1.0D0)*2.0D0)
     END IF
